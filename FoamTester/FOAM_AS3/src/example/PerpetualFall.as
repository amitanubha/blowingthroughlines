package example
{
	...
	public class PerpetualFall extends Sprite
	{
		...
		public function PerpetualFall()
		{
			...
			//build the ramp
			var ramp:RigidBody = new RigidBody(250, 400, Simplification.INFINITE_MASS, ShapeUtil.createRectangle( 500, 30 ));
//			ramp.vertices = ShapeUtil.createRectangle( 500, 30 );
			ramp.friction = 200; // doesn't seem to affect sliding...
			ramp.elasticity = 0.25; // doesn't seem to affect others bouncing off
			ramp.q = 0.4;
			ramp.av = 0;
			
			//trace(ramp.x, ramp.y, ramp.rotation.theta*(180/Math.PI), ramp.position.x, ramp.position.y);
			
			// test shape to check rotation
			var rampO:Shape = new Shape();
			rampO.x = ramp.x;
			rampO.y = ramp.y;
			rampO.rotation = ramp.rotation.theta*(180/Math.PI)
			rampO.graphics.beginFill(0xbada55, 0.7);
			rampO.graphics.drawRect(-250, -15, 500, 30);
			this.addChild(rampO);
			
			
			foam.addElement( ramp );
			...
			var spring:RigidBodyBungee = new RigidBodyBungee( swing, new Vector( -85, 0 ), anchor1, new Vector( 0, 1 ), 0.05 );
			foam.addRenderable( new Renderable( spring ) );
			
			//create a bungee between swing and anchor2 and add it to be rendered
			spring = new RigidBodyBungee( swing, new Vector( 85, 0 ), anchor2, new Vector( 0, 1 ), 0.01 );
			foam.addRenderable( new Renderable( spring ) );
